...
Reader links remain clean hash routes for the SPA, but social-preview crawlers cannot see a URL fragment. The home page has crawlable metadata in `index.html`, while subjects, schedule, resources, course pages, and every published lecture also receive generated crawlable aliases (for example `/courses/`, `/schedule/`, `/course/<routeSlug>/`, and `/lecture/<id>/`). Each alias contains context-specific Open Graph/Twitter title, description, image, and canonical URL, then redirects human visitors to the matching hash route. Run `npm run generate:share-pages` before a release (the queue publication path does this automatically), and keep the generated aliases in the release commit so WhatsApp and other link-preview clients show the actual page title rather than the generic site title.
