...
## Schedule display contract and live-source exception

### Explicitly authorized live-capture publication

When the owner explicitly requests publication from a finalized live capture without waiting for the provider upload, a reviewed lecture may omit `recordingUrl` and carry `sourceEvidence: { kind: "live-capture", captureFinalized: true, reviewComplete: true, recordingLinkStatus: "pending", timebase: "local-capture" }`. This exception requires saved video, matching metadata, reviewed instructional boundaries and all normal note requirements. The public note must explain its local-capture timebase. The archive labels the provider link as pending and offers the verified subject folder, never a fake Watch Recording link. Later attach the verified Stream URL and remove the pending source state without changing the stable publication id or timecodes silently. Raw capture and participant data remain private. Ordinary provider-recording publications still require a Stream URL.
