...
The dedicated recording and note-taker profile is [`agents/recording-note-taker.md`](agents/recording-note-taker.md). Future processing must follow it for the approved full bilingual lecture-note presentation, evidence-backed web/video/practice research, and the mandatory per-lecture subject-resource refresh.
...
- Every lecture is one bilingual publication unit in `data/lectures/<lecture-id>.js`, shaped as `{ en: {...}, hi: {...} }` and aggregated by `data/lecture-notes.js`. Create, review, validate, and publish the pair together; never create separate English-only, Hindi-only, or duplicate bilingual editions. Preserve matching structure, timestamps, formulas, URLs, and quiz answer indexes across the two columns. Keep established technical terms in English when a Hindi translation would reduce precision.
- Bilingual source data is rendered as one aligned bilingual note surface: substantive lecture content appears in English and Hindi together, side by side on desktop and sequentially on mobile. Do not alternate whole cards between languages or collapse the note into Hindi alone. Compact labels, chips, technical names, and metadata may render once when Hindi adds no meaning; never delete either source column or weaken its validation parity.
- Mini-capstones are public educational material. Keep one complete bilingual idea per published lecture in tracked `data/capstones.js`, validate English/Hindi parity, and render it directly in the lecture note. Never add usernames, access codes, credential verifiers, authentication UI, or private learner data to the capstone module or deployed site. Draft/source material may remain under ignored `.course-data/`, but the tracked module is the deployable source of truth.
- All public pages use the clean, language-neutral hash routes such as `#/`, `#/course/eai-6401-reinforcement-learning`, and `#/lecture/<id>`. Substantive lecture notes render as aligned bilingual English/Hindi content by default; compact page surfaces remain selective where a second copy adds no value. Do not add a language selector or publish `/en`, `/hi`, or `/bi` route choices. Legacy prefixed links and code-only course/resource links may redirect to the equivalent clean route.
