...
function signalCards(notes, text, readerCopy = hinglishCopy) {
  ...
  return signalLabels.map(([key, label]) => {
    ...
    const suggestedItems = notes.en.suggestedPractice?.[key] || [];
...
// Older note records stored insight copy as plain strings. Normalize both
// shapes at the renderer boundary so a valid insight can never silently
// become an empty card when a record is opened in production.
function normalizedInsight(item, index) {
  if (typeof item === "string") {
    const body = item.trim();
    const firstSentence = body.split(/[.!?](?:\s|$)/)[0]?.trim();
    return {
      label: "Connection",
      title: firstSentence?.length > 4 ? firstSentence : `Applied insight ${index + 1}`,
      body,
    };
  }
  return {
    label: item?.label || "Insight",
    title: item?.title || `Applied insight ${index + 1}`,
    body: item?.body || "",
  };
}
...
function renderLecture(id, ctx) {
  ...
  main.innerHTML = `<header class="lecture-hero"><div class="lecture-hero__meta"><a class="back-link" href="${href(lang, coursePath(found.course))}">← ${escapeHtml(found.course.code)}</a><span>${text.lecture} ${String(found.number).padStart(2, "0")}</span><span>${escapeHtml(found.displayDate)}</span></div>${compactBilingualCopy(en.title, hi.title, "h1")}${bilingualCopy(en.lede, hi.lede, "div", "lecture-hero__lede")}<div class="lecture-hero__actions"><button class="button button--quiet" type="button" data-share-note="${escapeHtml(found.id)}">Share note ${icon("external")}</button></div><dl><div><dt>${text.recording}</dt><dd>${escapeHtml(found.duration)}</dd></div><div><dt>${text.instructionalInterval}</dt><dd>${escapeHtml(en.instructionalInterval)}</dd></div><div><dt>${text.reviewLevel}</dt><dd>${compactBilingualCopy(en.reviewLevel, hi.reviewLevel)}</dd></div></dl></header>
    <div class="notes-layout"><aside class="notes-toc" id="notes-toc"><button class="notes-toc__toggle" type="button" aria-expanded="true" aria-controls="notes-toc-panel"><span class="notes-toc__toggle-label">${text.onThisPage}</span><span class="notes-toc__toggle-icon" aria-hidden="true">☰</span></button><div class="notes-toc__panel" id="notes-toc-panel"><nav>${navItems.map(([anchor, label]) => `<a href="#${anchor}" data-scroll="${anchor}">${label}</a>`).join("")}</nav></div></aside><article class="notes-article">
      ...
      <section id="insights" class="notes-section"><p class="eyebrow">${text.addedAnalysis}</p><h2>${text.insightsConnections}</h2><div class="insight-list">${insightItems.map((item, index) => `<article><span>${escapeHtml(item.label)}</span>${compactBilingualCopy(item.title, item.title, "h3")}${bilingualCopy(item.body, item.body, "p")}</article>`).join("")}</div></section>
...
function historyMeeting(meeting) {
  ...
  return `<div class="time-zone-panel history-meeting"><strong>${escapeHtml(meeting.label || "Lecture")}</strong><div class="time-zone-panel__primary"><span>India · IST</span><strong>${formatWallClock(meeting.start)} – ${formatWallClock(meeting.end)}</strong><small>${days}</small></div><small>${range}</small><div class="history-ct"><span class="eyebrow">Central Time · America/Chicago</span>${[...variants].map(([text, group]) => `<p>${escapeHtml(text)}${dated ? `<small>IST dates ${group.first} – ${group.last}</small>` : ""}</p>`).join("")}</div>${dated ? `<details class="history-dates"><summary>View ${dates.length} scheduled dates in IST and CT</summary><p>Calculated from the published recurrence; cancellations and attendance are not recorded here.</p><ol>${dates.map(date => {
...
function renderSemesterHistory(ctx) {
  main.innerHTML = `<section class="page-intro"><p class="kicker"><span></span>Semester 1 → Semester 4</p><h1>Four semesters, one class history</h1><div class="page-intro__copy"><p>Subject pages, lecture and lab schedules, and recording folders. India time (IST) is the base; Central Time (CT) is calculated for America/Chicago, including daylight saving and previous-day changes.</p><p>Moodle links checked 12 September 2026. Semester 4 also includes all nine subjects from the existing schedule. Institutional sign-in may be required for source links.</p></div><a class="button button--quiet" href="#/schedule">Current weekly schedule →</a></section>
    <nav class="history-nav section-shell" aria-label="Jump to semester">${semesterHistory.map(semester => `<a class="history-nav__item history-semester--${semester.semester}" href="#semester-${semester.semester}" data-scroll="semester-${semester.semester}"><span>0${semester.semester}</span>Semester ${semester.semester}${icon("arrow")}</a>`).join("")}</nav>
    <div class="section-shell history-sections">${semesterHistory.map(semester => `<details class="history-semester history-semester--${semester.semester}" id="semester-${semester.semester}"><summary class="history-semester__summary"><span class="history-semester__marker">S${semester.semester}</span><span class="history-semester__title"><span class="eyebrow">${semester.period}</span><h2>Semester ${semester.semester}</h2><span class="history-semester__description">Subjects, recordings & class schedules · IST + CT</span></span><span class="history-semester__meta"><span>${semester.courses.length} subjects</span>${icon("arrow")}</span></summary><div class="history-grid">${[...semester.courses].sort(compareHistoryCourses).map(course => {
      ...
      const accent = existing?.accent || ["cobalt", "teal", "amber", "violet"][semester.semester - 1];
      const monogram = existing?.icon || course.title.split(/\s+/).filter(word => !["and", "of", "in"].includes(word.toLowerCase())).slice(0, 2).map(word => word[0]).join("");
      return `<article class="course-card course-card--${accent} history-course">
        <div class="course-card__top"><span class="course-monogram">${escapeHtml(monogram)}</span><span class="status ${recordings ? "status--active" : "status--waiting"}"><i></i>${recordings ? "Recordings available" : "Project workspace"}</span></div>
